...
using MoonDriverDotNET.Compiler;
using System;
using System.IO;
...
namespace MoonDriverDotNET.Console
{
    class Program
    {
                    ...
                    if (!string.IsNullOrEmpty(srcFile))
                    {
                        destFileName = Path.Combine(Path.GetDirectoryName(Path.GetFullPath(srcFile))
                            , string.Format("{0}{1}"
                                ...
                                , Common.Common.objExtension)
                            );
                    }

                    compiler.work.in_name = srcFile;
                    compiler.work.out_name = Path.Combine(Path.GetDirectoryName(Path.GetFullPath(srcFile)), string.Format("{0}{1}"
                                , Path.GetFileNameWithoutExtension(srcFile)
                                , ".h"));
